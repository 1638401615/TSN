...
void ForwardFunction::forward(const unsigned char* destMac, void* data, size_t len, RELAY_ENTITY type) {
    INFO("Forward on call");

    /* get outbound ports */
    std::vector<unsigned short> ports = MacTable::findOutboundPorts(destMac);
